...
Local Computer: On our local computer, we notice an initial speedup on 2 threads of .816, with a gradual decline on 3 and 4 threads, where the best speedup we acheive is seen to be ~.70 on 4 threads. After this point, the speedup still remains below 1, but gradually begins to rise as we increase the thread count. As the thread count increases above 4, we may run into more threads concurrently observing previously unvisited nodes, and thus having increased contention and locking that begins to outweigh the increased loop parallelism benefits. 

School Cluster: On the school cluster, we notice a very small initial speedup on 2 threads, this time higher than on the local computer at only .96. We do notice a similar pattern with the gradual improvement on 3 and 4 threads, however the speedup still remains higher than on the local computer as it only reaches .815 (which is almost the same as Locally we got with only 2 threads). The speedup improvement stops for 5 threads with an almost identical speedup, before again similarly rising again as thread count increases further. The point where speedup gains stops seems to be consistent with both local and school cluster - pointing to 4 threads as the point in this code setup where thread contentions starts to dominate increased loop parallelism.
...
Local Computer: For the task speedup, we notice different speedup results than with parallel. The initial speedup is, while below 1, negligibly so, with just a .98 speedup for 2 threads - which is quite higher than the .816 we observed on local with the parllel method. However we notice the gradual decrease in speedup continue for much longer through the task model, as we notice decreasing speedup all the way until 32 threads, where we acheive our best speedup of .643 - which eclipses the best speeduo from the parallel loop method. From here, the speedup again flattens out with very small speedup observed for 48 and 64 threads.
